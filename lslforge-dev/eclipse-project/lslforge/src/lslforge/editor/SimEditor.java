...
public class SimEditor extends EditorPart implements NodeListener {
		...
		protected CellEditor getCellEditor(Object element) {
			...
			if (n.hasValueChoices()) {
			    ...
			    if ("scripts".equals(n.getChoicesId())) { //$NON-NLS-1$
			        ...
			        final LinkedList<String> avnames = new LinkedList<>();
   ...
   private class SimNameEditingSupport extends EditingSupport {
		...
		protected void setValue(Object element, Object value) {
            ...
            if (!status.isOk() || value.equals(n.getNameDisplay())) return;
			...
			public void menuAboutToShow(IMenuManager manager) {
				...
				if (factories.length > 0) {
	                ...
	                for (NodeFactory factory : factories) {
	                    manager.add(new AddNodeAction(factory));
	                }
				...
				if (parent != null) {
				    ...
				    if (factories.length > 0) {
                        ...
                        for (NodeFactory factory : factories) {
                            addItemBeforeSubMenu.add(new AddNodeBeforeAction(factory));
                            addItemAfterSubMenu.add(new AddNodeAfterAction(factory));
                        }
	...
	public void nodeStructureChanged(final Node n) {
        asyncExec(new Runnable() {  @Override
		public void run() {
