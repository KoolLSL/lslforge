...
//				return new Region(selection.x, selection.y);
	...
	private static synchronized HashMap<String,String> getHoverInfo() {
		if (hoverInfo == null) {
			...
			for (LSLHandler handler : handlers) {
				hoverInfo.put(handler.getName(), handler.fullDescription());
			}

			LSLFunction functions[] = LSLForgePlugin.getDefault().getLSLMetaData().getFunctions();
			for (LSLFunction function : functions) {
				hoverInfo.put(function.getName(), function.fullDescription());
			}
		}
		return hoverInfo;
	}
