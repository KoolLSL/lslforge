...
public class LSLCompletionProcessor implements IContentAssistProcessor {
	...
	private static final Comparator<ICompletionProposal> PROPOSAL_COMPARATOR = new Comparator<>() {
		@Override
	...
	private static String formatParams(LSLParam[] params) {
		...
		for (LSLParam param : params) {
			buf.append(sep);
			sep = ", "; //$NON-NLS-1$
			buf.append(param.getType()).append(" ").append(param.getName()); //$NON-NLS-1$
		}
	...
	private static String formatArgs(LSLParam[] params) {
		...
		String sep = ""; //$NON-NLS-1$
		for (LSLParam param : params) {
			buf.append(sep).append(param.getName());
			sep = ", "; //$NON-NLS-1$
		}
		return buf.append(")").toString(); //$NON-NLS-1$
	}
	...
	public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer, int documentOffset) {
		...
		for (CompletionInfo prop : possibleProposals) {
