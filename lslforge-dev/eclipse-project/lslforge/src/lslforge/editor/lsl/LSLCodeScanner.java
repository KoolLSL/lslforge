...
public class LSLCodeScanner extends RuleBasedScanner implements ColorProviderListener, IPropertyChangeListener {
    ...
    private static final HashMap<String,Integer> STYLE_TO_CONST = new HashMap<>();
    ...
    private HashSet<ScannerChangeListener> listeners = new HashSet<>();
    ...
    public LSLCodeScanner(LSLColorProvider provider, String[] handlerNames,
        ...
        for (String element : STYLE_KINDS) {
            for (String element2 : STYLES) {
                store.setDefault(element + '.' + element2, false);
            }
        }
    ...
    private int computeBits(String s0) {
        ...
        for (String element : STYLES) {
            x |= computeBit(store.getBoolean(mkId(s0,element)), STYLE_TO_CONST.get(element).intValue());
        }
        return x;
    }
