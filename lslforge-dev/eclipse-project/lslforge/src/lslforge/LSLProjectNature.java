...
public class LSLProjectNature implements IProjectNature, IResourceChangeListener {
    ...
    private static class BetterDeltaVisitor implements IResourceDeltaVisitor {
		...
		private final LinkedList<IResource> newDerivedResources = new LinkedList<>();
		private final LinkedList<LSLForgeElement> addsAndUpdates = new LinkedList<>();
		private final LinkedList<LSLForgeElement> removals = new LinkedList<>();
		...
		public boolean visit(IResourceDelta delta) throws CoreException {
			...
			if (resource != null) {
				...
				LSLDerivedScript script = resource.getAdapter(LSLDerivedScript.class);
				...
				if (element != null) {
					...
					if (script != null && delta.getKind() == IResourceDelta.ADDED) {
						...
						IProject p = resource.getAdapter(IProject.class);
	...
	private class SourceListBuilder implements IResourceVisitor {
		...
		private final HashMap<String,String> moduleMap = new HashMap<>();
		private final HashMap<String,String> moduleNameToPath = new HashMap<>();
		private final HashMap<String,String> scriptMap = new HashMap<>();
		private final HashMap<String,String> scriptNameToPath = new HashMap<>();
...
//		private Item[] scripts;
	...
	private final HashSet<RecompileListener> recompListeners = new HashSet<>();
...
//						Result r = cserver.execute(cmd, extra);
				...
				final HashMap<String, LinkedList<ErrInfo>> map = new HashMap<>();
				synchronized (this) {
