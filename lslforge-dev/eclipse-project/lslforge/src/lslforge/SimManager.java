...
public class SimManager implements SimEventListener {
    ...
    private HashSet<SimListener> listeners = new HashSet<>();
    private HashSet<SimEventListener> simEventListeners = new HashSet<>();
    private HashSet<SimMetaDataListener> simMetaDataListeners = new HashSet<>();
...
//                page.activate(activePart);
	...
	public void putEvent(final SimEvent event) {
        ...
        Thread t = new Thread() {
			...
			public void run() {
                try {
                    synchronized (simEventListeners) {
                        for (SimEventListener l : simEventListeners) {
                            l.putEvent(event);
                        }
                    }
                } catch (Exception e) {
    ...
    private void buildSimMetaData() {
        Job job = new Job("BuildSimMetaData") { //$NON-NLS-1$
			...
			protected IStatus run(IProgressMonitor monitor) {
                ...
                HashMap<String,SimEventDefinition> map = new HashMap<>();
    ...
    protected void fireSimMetaDataReady() {
        for (SimMetaDataListener l : simMetaDataListeners) {
            l.metaDataReady();
        }
    }
