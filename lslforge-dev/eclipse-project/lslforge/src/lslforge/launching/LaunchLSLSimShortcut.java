...
public class LaunchLSLSimShortcut implements ILaunchShortcut {
    ...
    private void adaptAndLaunch(String mode, Object o) {
        if (o instanceof IAdaptable) {
            ...
            IResource r = null;
            if ((s = ((IAdaptable)o).getAdapter(LSLForgeScript.class)) != null) {
                r = s.getResource();
            } else if ((w = ((IAdaptable)o).getAdapter(SimProject.WorldNode.class)) != null) {
                r = w.getResource();
            }
            if (r != null) {
                try {
	...
	private ILaunchConfiguration findConfig(IResource r) throws CoreException {
		...
		for (ILaunchConfiguration config2 : configs) {
			if (config2.getAttribute(LC_RESOURCE_NAME, "").equals(name)) { //$NON-NLS-1$
				config = config2;
				break;
			}
		}
